...
const GIFT_CARDS = readJsonEnv("GIFT_CARD_PRODUCTS_JSON", [
  { id: "us5", title: "Apple Gift Card — US $5", price: 55, currency: "GHS", region: "US" },
  { id: "us10", title: "Apple Gift Card — US $10", price: 105, currency: "GHS", region: "US" },
  { id: "us25", title: "Apple Gift Card — US $25", price: 255, currency: "GHS", region: "US" }
]);
...
async function cryptoMenu(chatId, order) {
  const options = [
    ["₿ BTC", "BTC", process.env.BTC_ADDRESS],
    ["💵 USDT TRC20", "USDT-TRC20", process.env.USDT_TRC20_ADDRESS],
    ["💵 USDT ERC20", "USDT-ERC20", process.env.USDT_ERC20_ADDRESS],
    ["🔷 ETH", "ETH", process.env.ETH_ADDRESS],
    ["🔺 TRX", "TRX", process.env.TRX_ADDRESS]
  ].filter(x => x[2]);

  if (!options.length) {
    await send(chatId, "<b>Crypto payments are not configured yet.</b>\nAsk the admin to add wallet addresses.", {
      reply_markup: kb([[btn("⬅️ Back", "menu:home")]])
    });
    return;
  }

  await send(chatId, `<b>₿ CHOOSE CRYPTO</b>\n\nOrder: <code>${order.id}</code>\nAmount to pay: <b>GH₵${order.amount}</b>\n\nSelect the network you want to use:`, {
    reply_markup: kb(options.map(x => [btn(x[0], `crypto:${order.id}:${x[1]}`)]).concat([[btn("⬅️ Cancel", `cancel:${order.id}`)]]))
  });
}
...
async function handleCallback(q) {
  ...
  if (data.startsWith("crypto:")) {
    ...
    const address = map[network];
    if (!address) return send(chatId, "That network is not configured.");
    order.cryptoNetwork = network; order.cryptoAddress = address; order.status = "AWAITING_CRYPTO_PAYMENT";
    order.updatedAt = nowISO(); await putJSON(keyOrder(order.id), order);
    await send(chatId, `<b>₿ CRYPTO PAYMENT</b>
...
Then tap <b>I've Paid</b>. The admin will manually verify the transaction before fulfillment.`, {
      reply_markup: kb([[btn("✅ I've Paid", `cryptopaid:${order.id}`)], [btn("❌ Cancel", `cancel:${order.id}`)]])
    });
    return;
  }
  ...
  if (data === "store:gift") {
    const rows = GIFT_CARDS.map(p => [btn(`🎁 ${p.title} — GH₵${p.price}`, `gift:${p.id}`)]);
    rows.push([btn("⬅️ Back", "menu:store")]);
    return send(chatId, "<b>🎁 APPLE / iTUNES GIFT CARDS</b>\n\nChoose a region/denomination:", { reply_markup: kb(rows) });
  }
  ...
  if (data.startsWith("gift:")) {
    ...
    return send(chatId, `<b>🎁 ${esc(p.title)}</b>\n\nPrice: <b>GH₵${p.price}</b>\nOrder: <code>${order.id}</code>\n\nSend the account/region details needed for fulfillment (for example the store region). Do not send passwords or private account credentials.`);
  }
...
async function adminPending(chatId) {
  ...
  for (const k of keys) {
    ...
    if (o && ["PAYMENT_REVIEW","CRYPTO_REVIEW","AWAITING_PAYMENT","AWAITING_CRYPTO"].includes(o.status)) orders.push(o);
  }
